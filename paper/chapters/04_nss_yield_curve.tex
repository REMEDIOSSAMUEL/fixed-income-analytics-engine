...
The Nelson--Siegel model combines level, slope-related and curvature contributions. Svensson's extension adds another curvature contribution, giving the Nelson--Siegel--Svensson (NSS) model \parencite{nelson1987,svensson1994}. The parameter vector is
\(\theta=(\beta_0,\beta_1,\beta_2,\beta_3,\tau_1,\tau_2)\).
The \(\beta\) coefficients are decimal rates and the positive decay scales \(\tau_1,\tau_2\) are measured in years. A factor loading is a function of maturity multiplying a coefficient. For either decay scale \(\tau\), dimensionless loadings define the generic decimal rate \(r_{\mathrm{NSS}}\):
\begin{align}
...
For dimensionless \(x=T/\tau\), subtracting nearly equal rounded numbers in \(1-e^{-x}\) causes cancellation \parencite{higham2002}. The implementation uses \texttt{expm1}, accurately evaluating \(e^{-x}-1\), and separate fifth-degree loading expansions below \(x=10^{-3}\).
...
Nonlinear optimisation searches for parameters minimising \(S\); local minima minimise it within a neighbourhood, without ensuring global optimality. Six deterministic starts reduce starting-point dependence (\cref{sec:optimiser}). Identification is the ability of the observations and model restrictions to distinguish parameter values; it is weak when materially different values fit almost equally well. A small fitting error does not imply unique identification \parencite{gilli2010}: a constant curve leaves decay scales undetermined. If \(\tau_1=\tau_2\), only the sum \(\beta_2+\beta_3\) enters their shared loading. Near equality can therefore produce unstable coefficients even when fitted rates change little.
