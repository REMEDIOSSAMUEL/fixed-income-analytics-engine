...
Nonlinear optimisation searches for parameters minimising \(S\); local minima minimise it within a neighbourhood, without ensuring global optimality. Six deterministic starts reduce starting-point dependence (\cref{sec:optimiser}). A small fitting error does not imply that parameters are uniquely identified \parencite{gilli2010}: a constant curve leaves decay scales undetermined. If \(\tau_1=\tau_2\), only the sum \(\beta_2+\beta_3\) enters their shared loading. Near equality can therefore produce unstable coefficients even when fitted rates change little.
